...
app = FastAPI(title="Cofer One IA - Cofer U Pass Bridge", version="0.3.0")
...
async def _ensure_model_available(model: str) -> None:
    ...
    if model not in {str(profile.get("profile_id")) for profile in profiles}:
        raise HTTPException(status_code=503, detail=f"no active Cofer U Pass worker for profile: {model}")
...
async def root(_: None = Depends(auth)) -> dict[str, Any]:
    return {"name": "cofer-u-pass-bridge", "version": app.version, "mode": "text-file-exchange"}


@app.get("/health")
async def health(_: None = Depends(auth)) -> dict[str, Any]:
    profiles = await asyncio.to_thread(store.active_profiles)
    return {"status": "ok", "workers_profiles": profiles, "jobs": await asyncio.to_thread(store.stats)}
...
async def models(_: None = Depends(auth)) -> dict[str, Any]:
    ...
    return {
        "object": "list",
        "data": [
            {"id": p["profile_id"], "object": "model", "created": 0, "owned_by": f"cofer-u-pass:{p.get('provider','web')}"}
            for p in profiles
        ],
    }
...
async def upload_file(
    ...
    try:
        meta = await asyncio.to_thread(
            store.put_file, file.file, filename=file.filename or "upload.bin", purpose=purpose,
...
async def responses(request: Request, _: None = Depends(auth)):
    ...
    job_id = await asyncio.to_thread(store.create_job, body)
...
async def chat_completions(request: Request, _: None = Depends(auth)):
    ...
    if not model:
        raise HTTPException(status_code=400, detail="model is required")
    await _ensure_model_available(model)
    job_id = await asyncio.to_thread(store.create_job, converted)
    response = await _wait_job(job_id)
    ...
    async def stream() -> AsyncIterator[str]:
        ...
        yield "data: " + json.dumps({
            "id": chunk_id, "object": "chat.completion.chunk", "created": final["created"], "model": final["model"],
...
async def next_job(
    worker_id: str = Query(...), profiles: str = Query(...), _: None = Depends(auth)
):
    ...
    while time.monotonic() < deadline:
        try:
            ...
            raise HTTPException(status_code=409, detail="worker must register before polling")

...
ensure_cofer_secrets(){ for key in LITELLM_MASTER_KEY LITELLM_SALT_KEY LITELLM_DB_PASSWORD LITELLM_UI_PASSWORD;do local value; value="$(read_env "$key"||true)"; if [[ -z "$value"||"$value" == CHANGE_ME* ]];then set_env "$key" "$(random_secret)"; echo "Generated $key";fi;done; }
